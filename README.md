<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:2563EB,100:60A5FA&height=200&section=header&text=Jainer%20Yohany%20Chocue%20Viluche&fontSize=42&fontColor=ffffff&animation=fadeIn&fontAlignY=38&desc=Systems%20Engineering%20%7C%20Python%20%7C%20AI%20Automation&descAlignY=58&descSize=18" alt="Banner animado" />

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=24&duration=3000&pause=1000&color=2563EB&center=true&vCenter=true&width=600&lines=Estudiante+de+Ingenier%C3%ADa+de+Sistemas;Python+%7C+Automatizaci%C3%B3n+con+IA;Creador+de+%40lavoztech" alt="Texto animado" />

<br><br>

<a href="https://github.com/jainerchocue">
  <img src="https://img.shields.io/badge/GitHub-jainerchocue-2563EB?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />
</a>
<a href="https://www.linkedin.com/in/jainer-johany-chocue-viluche-710b54267">
  <img src="https://img.shields.io/badge/LinkedIn-Conectar-2563EB?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
</a>
<a href="https://www.tiktok.com/@lavoztech">
  <img src="https://img.shields.io/badge/TikTok-@lavoztech-2563EB?style=for-the-badge&logo=tiktok&logoColor=white" alt="TikTok - La Voz Tech" />
</a>

</div>

<br>

## Sobre mí

Soy estudiante de **Ingeniería de Sistemas** en la **Corporación Universitaria Comfacauca (Unicomfacauca)**, en Popayán, Colombia.

Mi enfoque está orientado al desarrollo de software, la automatización mediante Inteligencia Artificial y la divulgación tecnológica. Trabajo en el diseño de soluciones eficientes utilizando herramientas de vanguardia, combinando la experimentación técnica con el aprendizaje dentro de comunidades de desarrolladores.

<br>

## Comunidad y presencia digital — La Voz Tech

Soy el creador y administrador de **La Voz Tech** (`@lavoztech`), un espacio en TikTok dedicado a:

- Divulgación de tips tecnológicos y noticias del sector IT.
- Enseñanza accesible de lógica de programación y desarrollo de software.
- Construcción de una comunidad de aprendizaje colaborativo.

<br>

## Tecnologías y competencias

<div align="center">

<img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" alt="Python" />
<img src="https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white" alt="MySQL" />
<img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />

<br>

<img src="https://img.shields.io/badge/OpenAI-412991?style=for-the-badge&logo=openai&logoColor=white" alt="OpenAI API" />
<img src="https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white" alt="pandas" />
<img src="https://img.shields.io/badge/Matplotlib-11557C?style=for-the-badge&logo=python&logoColor=white" alt="Matplotlib" />

</div>

<br>

**Inteligencia Artificial avanzada:** Prompt Engineering · AI Agents · Modelos de Lenguaje (LLMs) · OpenAI API

**Conceptos clave:** Lógica de programación · Estructuras de datos · Arquitectura de software (MVC) · Automatización de procesos

**En aprendizaje activo:** Análisis y visualización de datos · Metodologías DevOps

<br>

## Proyectos y participaciones

Me apasiona transformar ideas creativas en código y automatizaciones reales. He participado en escenarios de alto nivel como la **OpenAI Build Week** (organizada en Devpost), desarrollando herramientas como **Helio Architect**, la cual conecta IA con Blender para optimizar flujos de diseño en ingeniería y arquitectura a través de lenguaje natural.

Te invito a explorar mis [repositorios](https://github.com/jainerchocue?tab=repositories) para conocer el código fuente de mis proyectos de automatización, mis laboratorios de estudio open-source y mis desarrollos más recientes.

<br>

## Estadísticas de GitHub

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=jainerchocue&show_icons=true&theme=default&hide_border=true&title_color=2563EB&icon_color=2563EB&text_color=333333" alt="Estadísticas de GitHub" height="165" />

<br><br>

<img src="https://raw.githubusercontent.com/jainerchocue/jainerchocue/output/github-contribution-grid-snake.svg" alt="Animación de contribuciones" width="100%" />

</div>

<br>

## Conectemos

Si deseas colaborar en proyectos de IA, aportar a la comunidad o charlar sobre tecnología, puedes encontrarme en:

- **LinkedIn:** [jainer-johany-chocue-viluche-710b54267](https://www.linkedin.com/in/jainer-johany-chocue-viluche-710b54267)
- **TikTok (La Voz Tech):** [@lavoztech](https://www.tiktok.com/@lavoztech)

<br>

<div align="center">

*"La mejor forma de aprender es compartiendo el conocimiento."*

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:60A5FA,100:2563EB&height=100&section=footer" alt="Footer animado" />

</div>
